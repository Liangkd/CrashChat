...
class CrashAccidentCaptionDataset(BaseVideoEvalDataset):
    """
    Crash 视频事故描述（caption）评估 Dataset
      ...
      [
        {
          ...
          "conversations": [
            {
              "from": "human",
              "value": "<video>\\nYou are a traffic accident analyst. ..."
            },
            {
              "from": "gpt",
              "value": "ego-car hits a motorbike"
            }
          ],
          "source": "DADA2000"
        },
    ...
    data_root 预期为：例如 /home/kaidi/VideoLLaMA3/data
      其中包含：
        - crashchat_dada_video_accident_caption_test.json
        - videos/dada_xxx.mp4
    """

    MODAL: str = "video"  # 必须是 "video"，evaluate.py 会用到
    BENCHMARK_TYPE: str = "accident_caption"  # 名字随便，用于区分任务类型
    ...
    def load_data(self, data_root: str) -> Dict[int, Any]:
        ...
        这里先写死一个 accident caption 的 JSON 文件名：
          crashchat_dada_video_accident_caption_test.json

        后续如果需要 train/val，可以再改成根据 benchmark_name 或额外参数来选文件名。
        """
        json_file = os.path.join(
            data_root,
            "crashchat_dada_video_total_accident_caption_test.json"
        )
        ...
        for rec in records:
            ...
            meta_data = {
                ...
                "task_type": "crash_accident_caption",
                "ground_truth": gt_caption,  # str
    ...
    def evaluate(
          ...
          [
            {
              ...
              "prediction": pred_caption,  # 经过 process_response 的 caption（str 或 None）
            },
            ...
          ]
        ...
        for item in results:
            ...
            pred_caption = item["prediction"]

            # 某些样本可能解析失败（prediction=None）
            if pred_caption is None:
                continue

            gt_caption = self.data_dict[data_id]["ground_truth"]
