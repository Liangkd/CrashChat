...
"""
Compute BLEU, ROUGE-1/2/L and BERTScore for causal_reasoning results.

Assume the prediction JSON file is a list, with each element structured as follows:
{
"data_id": 134,
"video_path": "...",
"prompt": "...",
...
PRED_JSON = (
    "/outputs/causal_reasoning_evaluation_results/independent_monotask_models_causal_reasoningn/crashchat_dada_video_total_causal_reasoning_test_predict.json"
)

OUTPUT_DIR = (
    "/outputs/causal_reasoning_evaluation_results/independent_monotask_models_causal_reasoningn"
)

OUTPUT_TXT = os.path.join(
    OUTPUT_DIR,
    "crashchat_dada_video_causal_reasoning_test_metrics.txt"
)

OUTPUT_CSV = os.path.join(
    OUTPUT_DIR,
    "crashchat_dada_video_causal_reasoning_test_metrics.csv"
)
...
def load_refs_preds(records: List[Dict]) -> Tuple[List[str], List[str]]:
    ...
    if not refs or not preds:
        raise ValueError("No valid (gt_cause, pred_cause) pairs found.")
