...
class DataArguments:
    ...
    eval_data_path: Optional[List[str]] = field(default=None, metadata={"help": "Path to the validation data."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    early_stopping_threshold: float = field(
        default=0.001,
        metadata={"help": "Minimum improvement in eval_loss to be considered as an actual improvement."}
    )
...
def train(attn_implementation=None):
    ...
    if model_args.vision_encoder is not None:
        ...
        if local_rank == 0:
            def count_params(module, name):
                ...
                print(f"[DEBUG] {name}: trainable {trainable}/{total}")
            base_model = model.get_model()
            count_params(base_model, "LLM_total")
            count_params(base_model.vision_encoder, "vision_encoder")
            count_params(base_model.mm_projector, "mm_projector")
    ...
    if getattr(training_args, "early_stopping_patience", None) is not None and training_args.early_stopping_patience > 0:
